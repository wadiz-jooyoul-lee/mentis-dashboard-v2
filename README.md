...
실제 `$DOBBY_META`를 건드리지 않고, 격리된 `./.demo-meta`에 예시 오더를 심어 실행합니다.

```bash
npm run demo         # .demo-meta에 fixture 생성 후 DOBBY_META_PATH로 가리켜 dev 실행
npm run demo:seed    # fixture만 생성
npm run demo:clean   # .demo-meta 삭제
```

- 데모 데이터: K=1 code(종료)·K≥2(리뷰중, 변경 diff)·비소스(TASK)·최소(착수) 오더 + 잡 로그 예시. 모든 탭을 둘러볼 수 있습니다.
- `npm run dev`는 **실제** `$DOBBY_META`를 읽으므로 데모 데이터가 섞이지 않습니다. (`.demo-meta`는 git 무시)

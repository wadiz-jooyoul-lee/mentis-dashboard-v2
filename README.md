...
| `/orders` | 오더 목록(키·타입·work-type·단계·팬아웃 K·통과율·갱신). 날짜별 접이식 |
| `/orders/[key]` | 오더 상세 — **단일 라우트 + 탭**: 개요 / 분석 / 구현·산출 / 검증(code) / 에이전트(K≥2) / 변경(K≥2) / 종료 |
| `/agents` | 도비 에이전트 소개 |
...
- 잡 실행(`dobby-order` 헤드리스 spawn)은 이 버전(읽기 전용)에는 없습니다 — 2차 예정.

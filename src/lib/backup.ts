/**
 * Claude 세션 전사(~/.claude/projects) 전체 압축 백업 상태/실행. (서버 전용, node I/O)
 * 기준점 = "가장 최신 백업 아카이브 파일명의 시각"(별도 마커 없음) — 스크립트와 동일 규약.
 * 스크립트는 매번 전체를 담고 최신 하나만 남긴다(2026-09-09 전환). 전사는 덧붙기 전용이라
 * 증분이 같은 내용을 반복 저장했고, 아카이브 880MB 중 506MB가 같은 전사의 옛 판이었다.
 * 그래서 여기 `pending`은 "마지막 전체 백업 이후 바뀐 세션 수"를 뜻한다.

 ...
 */
export type IveExpr = "base" | "think" | "surprise" | "curious" | "smile";
...
// think·curious는 2026-08 추가 수집분(얼굴 중심 정사각 크롭)이라 focal이 전부 중앙(50% 50%)이다.
export const IVE_AVATARS: Record<string, IveCfg> = {
  가을: { slug: "gaeul", color: "#E8703A", real: "김가을", birth: "2002-09-24", pos: "메인댄서·리드래퍼",
    focal: { base: "50% 16%", smile: "50% 30%", surprise: "50% 34%", think: "50% 50%", curious: "50% 50%" } },
  안유진: { slug: "yujin", color: "#8E5BE8", real: "안유진", birth: "2003-09-01", pos: "리더·메인보컬",
    focal: { base: "46% 20%", smile: "52% 22%", surprise: "50% 18%", think: "50% 50%", curious: "50% 50%" } },
  레이: { slug: "rei", color: "#3AA0E8", real: "나오이 레이", birth: "2004-02-03", pos: "메인래퍼",
    focal: { base: "50% 18%", smile: "50% 30%", surprise: "52% 28%", think: "50% 50%", curious: "50% 50%" } },
  장원영: { slug: "wonyoung", color: "#E94F8A", real: "장원영", birth: "2004-08-31", pos: "비주얼·센터",
    focal: { base: "51% 18%", smile: "50% 24%", surprise: "44% 18%", think: "50% 50%", curious: "50% 50%" } },
  리즈: { slug: "liz", color: "#F2B01E", real: "김지원", birth: "2004-11-21", pos: "메인보컬",
    focal: { base: "50% 16%", smile: "55% 20%", surprise: "62% 34%", think: "50% 50%", curious: "50% 50%" } },
  이서: { slug: "leeseo", color: "#2FC4A6", real: "이현서", birth: "2007-02-21", pos: "리드댄서·막내",
    focal: { base: "53% 18%", smile: "48% 26%", surprise: "56% 38%", think: "50% 50%", curious: "50% 50%" } },
};
 ...
 * 대기=기본(base), 분석=생각(think), 구현·수정=놀람(surprise), 리뷰=호기심(curious), 완료=활짝(smile).
 */
export function iveExpr(state?: string): IveExpr {
  const s = (state ?? "").replace(/\s/g, "");
  if (/완료|해결|종료/.test(s)) return "smile";
